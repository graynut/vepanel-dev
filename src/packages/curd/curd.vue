<template>
    <div class="curd-wrp">
        ...
        <template v-else-if="status === 2">
            <div class="curd-page-stand" v-if="pageHeaderStyle>1">
                <div class="curd-dialog-error" v-loading="pageError===null" v-if="pageError!==false">{{pageError}}</div>
                <component ref="page" v-bind="pageProps" :is="pageCmpt" v-show="pageError!==null" v-if="!pageError"/>
            </div>
            <div :class="'curd-page-stand'+ (pageHeaderStyle ? ' curd-page-standfix' : '')" v-else>
        ...
        <template v-else>
            <div class="curd-body">
                <div class="curd-header">
                    ...
                    <div class="curd-search">
                        <el-input v-model="keyword" size="mini" @keyup.enter.native="$_search">
            ...
            <!--弹窗单页-->
            <el-dialog
                :custom-class="'curd-dialog'+(pageHeaderStyle>1 ? ' curd-dialog-noheader' : (pageHeaderStyle>0 ? ' curd-dialog-fullfix' : ''))"
                :append-to-body="true"
            ...
            >
                <slot slot="title" v-if="pageHeaderStyle<2">
                    <slot v-bind="pageHeaderData" :name="pageHeader" v-if="pageHeader"/>
                    <span class="curd-dialog-name" v-else>{{pageName}}</span>
                    <button
                        type="button"
...
*  由于内部使用了 this.$admin 和 this.$root, 仅用于 vepanel
*/
export default {
    name:'curd',
    data() {
        return {
            ...
            // 单页头部 pageHeaderStyle: 0-普通header, 1-固定header, 2-无header(需自行设计 返回/关闭弹窗 按钮)
            // pageHeaderStyle=1 在手机版, header 是不固定的, 仅 pc版 生效
            pageHeaderStyle:0,
            pageHeader:null,
            pageHeaderData:null,
        ...
        */
        ...
        /* 操作项数据格式
            [{
                ...
                header:String    在 headerStyle!=2 的情况下, 自定义 header 的 template，参见 tableColumns() 注释
            ...
            若为单页组件, 会自动注入一些信息到指定的组件中, 比如 rowRevise 操作就会注入操作行的 primary 字段值
    ...
    computed: {
        ...
        */
        ...
        tableColumns() {
            const columns = [];
            let hasFlex = false;
            const allowed = ['width', 'resizable', 'formatter', 'align', 'headerAlign'];
            const replaced = {
                cellClass: "className",
                headerClass:"labelClassName",
    ...
    methods:{
        ...
        $_doOperateAction() {
            ...
            if (type < 2) {
                this._operateData.dialog = Boolean(type);
            }
            // 操作按钮显示为 loading 状态
        ...
        $_initPage() {
            ...
            if (this._operateData.operate) {
                // 这是已载入列表的情况下, 点击操作项打开的, 直接使用缓存的
                operate = this._operateData.operate;
            } else {
                // 未载入列表, 直接访问操作页的 url (仅支持 顶部操作/单条数据操作, 底部操作不支持)
        ...
        $_loadPage(operate){
            ...
            if (dialog > 0) {
                ...
                //先显示弹窗,  在 loading 界面中等待单页组件加载
                this.dialogVisible = true;
            }
            // 设置 header 显示方式
            this.pageHeaderStyle = operate.headerStyle||0;
        ...
        // 加载单页组件, 会传入 props: {bag:this._operateData, page:this._operateMethods}
        $_toOperate(operate, dialog, finalComponent) {
            ...
            const pageName = 'name' in operate ? operate.name : '操作';
        ...
        $_compared(value, operator, match) {
            return curdUtils.curdCompared(value, operator, match)
        },
        ...
        options = {
            name: String, 标题头
            type: 0|1,    (0:打开普通单页; 1:打开弹窗单页)
            path:String|Object  component组件名称或组件对象
            headerStyle:int (0:普通模式; 1:固定模式; 2:省略模式)
            header:String   自定义 header 的 template 
            headerData:any  传递给自定义 header 的 props
...
.curd-list .el-table--border{
    border: none;
}

/*表格 header 来个阴影, 并修正复选框位置 */
...
/*重置 padding, 使表格更紧凑一点 */
.curd-list .el-table th{padding:1px 0 0 0}
.curd-list .el-table td, 
.curd-list .el-table--medium td{padding:8px 0}
...
.curd-page-stand .el-page-header{
    box-sizing: border-box;
    height:44px;
...
.curd-page-stand .el-page-header__left{
    align-items: center;
}
.curd-page-stand .el-page-header__content{
    flex:1;
}
.curd-page-component{
    flex:1;
}
.curd-page-standfix .el-page-header{
    width: 100%;
    margin:0;
    padding:10px 16px 9px 16px;
}
.curd-page-standfix .curd-page-component{
    overflow: auto;
}
...
.curd-dialog.is-fullscreen .el-dialog__header{
    margin: 0 16px;
}
.curd-dialog.is-fullscreen .el-dialog__body{
    flex:1;
}
.curd-dialog-fullfix .el-dialog__header{
    margin:0 !important;
}
.curd-dialog-fullfix .el-dialog__body{
    overflow: auto;
}
