        ...
        >
            <template slot="title">
                ...
                <draggable :list="selected" @end="_sortSelected" :animation="340" class="curd-choose-selected" v-if="multiple && selected.length">
                    <el-tag v-for="(tag,index) in selected" :key="tag.value+'_'+index" disable-transitions closable @close="_removeTag(index)">
...
const draggable = () => {
    return new Promise((resolve, reject) => {
        require(['vuedraggable'], resolve, reject)
    })
}

export default {
    name:'CurdChoose',
    // 在 el-form 内, 尝试继承 form 的 size 和 disable 属性
    inject: {
      elForm: {
        default: ''
      },
    ...
    props:{
        ...
        // 静态 icon, 将显示 elm icon 和 当前已加载的 iconfont icon
        // 开启此项后, 下面的就无需配置了
        staticIcon:Boolean,
    ...
    methods:{
        ...
        _select(option){
            ...
            if (!this.multiple) {
                ...
                (this.multipleRepeat || !this.selected.find(item => item.value === option.value))
            ) {
                update = true;
        ...
        _getCurrentIcons(){
            const icons = curdUtils.getAllIcons();
