...
><el-row :gutter="gutter">
    ...
    >
        <el-col v-if="!noFooter"><el-form-item>
            <el-button type="primary" :loading="submiting" @click="submitForm">{{submitText}}</el-button>
            <el-button @click="resetFields">{{resetText}}</el-button>
        </el-form-item></el-col>
    </slot>
</el-row></el-form>
</template>
...
props: 组件 component 支持的配置
   ...
   2.el-radio-group/el-checkbox-group  支持自动生成 solt: default
...
export default {
    ...
    computed:{
        fLabelPosition(){
            return this.forceSpan === 'xs' ? this.mLabelPosition : (
                this.forceSpan === 'sm' ? this.labelPosition : (
                    this.$root.phone ? this.mLabelPosition : this.labelPosition
                )
            )
        },
        ...
        form(){
            ...
            this.fields.forEach((field, index) => {
                ...
                if (!format.xspan || this.forceSpan === 'sm'){
                    format.xspan = format.span;
                } else if (this.forceSpan === 'xs') {
    ...
    methods:{
        ...
        _updateFormFieldsVhidden(inited){
            ...
            const fieldsVhidden = {};
            for (k in this._vshowCondition) {
                hide = !curdUtils.curdVshow(this.data, this._vshowCondition, k);
                if (inited) {
        ...
        submitForm(){
            this.validate().then(r => {
                ...
                return new Promise(done => {
                    this.$emit('submit', this.data, done)
                })
            }).catch(err => {
                // do nothing
            }).finally(err => {
...
.curd-elform .el-form-item--small .el-rate{
    height:32px;
}
.curd-elform .el-form-item--mini .el-rate{
    height:28px;
}
