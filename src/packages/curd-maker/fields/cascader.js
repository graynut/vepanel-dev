...
function clearDataBeforeChange(field, maker){
    ...
    const isValue = typeof v === 'object' && 'length' in v;
    if (!isValue || v.length > 0) {
        maker.addFieldData(field.name, []);
        maker.reCreateFiled();
    }
}

function onSourceChange(props, v, field, maker){
    let remote = {};
    if (props._source > 1) {
    ...
    props.remote = remote;
    //修改类型, ele ui 会 js 报错, 重建也没办法, 因为重建是在属性改变后
    //而属性改变一瞬间, 就已经报错了, 想完美处理, 只能修改 ele ui 了
...
const basicProps = {
    _source:{
        ...
        onChange(props, v, field, maker){
            setSupportSearch(props);
            onSourceChange(props, v, field, maker);
        }
    },
    options:{
        value:[],
